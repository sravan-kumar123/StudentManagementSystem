...
import random    #This module can be used to perform random actions such as
# generating random numbers, printing random a value for a list or string, etc.

import re    # check if a particular string matches a given regular expression
import sqlite3   #used to integrate the SQLite database with Python
import tkinter as tk   #create the object of widgets just by using the widget constructor
from tkinter import PhotoImage   #allows you to add user-defined images to your application
from tkinter.filedialog import askopenfilename, askdirectory  #create a file selection dialog window and return the filenames of the selected files.
#the initial default (current directory) when the Cancel button, or the X close button on the window, or if the user selects Esc

from tkinter.ttk import Combobox, Treeview   #combines a text field with a pop-down list of values
#a UI widget that presents data in hierarchical form, although non-hierarchical data is also supported

from tkinter.scrolledtext import ScrolledText  # to scroll the text
import smtplib #send emails
from email.mime.multipart import MIMEMultipart   #the transfer of single or multiple text and non-text attachments
...
login_student_icon: PhotoImage = tk.PhotoImage(file='login_student_img.png')
...
add_student_pic_icon = tk.PhotoImage(file='add_student_img.png')
...
def add_data(id_number, password, name, age, gender, phone_number,
             student_year, student_branch, email, pic_data):
    connection = sqlite3.connect('students_accounts.db')
    cursor = connection.cursor()
    cursor.execute(f"""
    INSERT INTO data VALUES('{id_number}','{password}','{name}','{age}', '{gender}',
     '{phone_number}','{student_year}', '{student_branch}', '{email}', ?)
     """, [pic_data])
...
def confirmation_box(message):
    ...
    message_lb = tk.Label(confirmation_box_fm, text=message, font=("times new roman", 15))
    ...
    yes_btn.place(x=190, y=160, width=75)
...
def draw_student_card(student_pic_path, student_data):
    ...
    data_font = ImageFont.truetype('bahnschrift', 17)

    draw.text(xy=(150, 60), text='Student_Card', fill=(0, 0, 0),
              font=heading_font)

    draw.multiline_text(xy=(5, 100), text=labels, fill=(0, 0, 0),
                        font=labels_font, spacing=6)

    draw.multiline_text(xy=(95, 100), text=student_data, fill=(0, 0, 0),
                        font=data_font, spacing=6)
...
def student_card_page(student_card_obj, bypass_login_page=False):
    ...
    print_student_card_btn.place(x=280, y=374, width=45, height=40)

    student_card_page_fm.place(x=50, y=30, width=400, height=450)


def welcome_page():
    # welcome page to student login page
    ...
    add_student_btn = tk.Button(welcome_page_fm, text='Create Account',
                                bg=bg_color, fg='white', font=('Bold', 15), bd=0, command=forward_to_add_account_page)
...
def student_dashboard(student_id):
    ...
    SELECT name, age, gender, year, branch, phone_number, email FROM data WHERE id_number == '{student_id}'
    """)
    ...
    def switch(indicator, page):
        home_btn_indicator.config(bg='lightblue')
        student_card_btn_indicator.config(bg='lightblue')
        security_btn_indicator.config(bg='lightblue')
        edit_data_btn_indicator.config(bg='lightblue')
    ...
    home_btn = tk.Button(options_fm, text='Home', font=('Bold', 15),
                         fg=bg_color, bg='lightblue', bd=0,
                         command=lambda:
                         switch(indicator=home_btn_indicator,
                                page=home_page))
    home_btn.place(x=10, y=50, width=90)

    home_btn_indicator = tk.Label(options_fm, bg=bg_color)
    home_btn_indicator.place(x=5, y=50, width=3, height=40)
    ...
    student_card_btn.place(x=10, y=110, width=90)

    student_card_btn_indicator = tk.Label(options_fm, bg='lightblue')
    student_card_btn_indicator.place(x=5, y=118, width=3, height=40)
    ...
    security_btn = tk.Button(options_fm, text='Security', font=('Bold', 15),
                             ...
                             switch(indicator=security_btn_indicator,
                                    page=security_page))
    security_btn.place(x=10, y=190, width=90)

    security_btn_indicator = tk.Label(options_fm, bg='lightblue')
    security_btn_indicator.place(x=5, y=190, width=3, height=40)
    ...
    edit_data_btn = tk.Button(options_fm, text='Edit Data', font=('Bold', 15),
                              ...
                              switch(indicator=edit_data_btn_indicator,
                                     page=edit_data_page))
    edit_data_btn.place(x=10, y=260, width=90)

    edit_data_btn_indicator = tk.Label(options_fm, bg='lightblue')
    edit_data_btn_indicator.place(x=5, y=260, width=3, height=40)

    logout_btn = tk.Button(options_fm, text='Logout', font=('Bold', 15),
                           fg=bg_color, bg='lightblue', bd=0, command=logout)
    logout_btn.place(x=10, y=330, width=90)
    ...
    def home_page():
        ...
        output.putalpha(mask)  # black background will disappear 'transparent'

        student_picture = ImageTk.PhotoImage(output)

        home_page_fm = tk.Frame(pages_fm)
        pic_img_obj: PhotoImage = ImageTk.PhotoImage(Image.open(student_pic))
        student_pic_lb = tk.Label(home_page_fm, image=pic_img_obj)
...
Year: {get_student_details[0][3]}\n
Branch: {get_student_details[0][4]}\n
contact: {get_student_details[0][5]}\n
Email: {get_student_details[0][6]}\n
"""

        student_details_lb = tk.Label(home_page_fm, text=student_details,
                                      font=('Bols', 13), justify=tk.LEFT)
...
"""
    ...
    def security_page():
        ...
        current_password_ent.place(x=60, y=80)
    ...
    def edit_data_page():
        ...
        def check_invalid_email(email):
            ...
            pattern = r"^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[_a-z0-9-]+)*(\.[a-z]{2,4})$"
        ...
        def check_inputs():
            ...
            if student_name_ent.get() == '':
                ...
                message_box(message='Student ph_Number is Required')
                ...
                selected_year = select_year_btn.get()
                selected_branch = select_branch_btn.get()
                ...
                cursor.execute(f"""
                UPDATE data SET name = '{name}', age='{age}', year='{selected_year}',
                branch='{selected_branch}', phone_number='{contact_number}', email='{email_address}'
                WHERE id_number == '{student_id}'
                """)
        ...
        add_pic_section_fm = tk.Frame(edit_data_page_fm, highlightbackground='gray',
        ...
        add_pic_btn.pack()
        add_pic_section_fm.place(x=5, y=5, width=115, height=105)

        student_name_lb = tk.Label(edit_data_page_fm, text='Student Full Name',
                                   font=('times new roman', 13))
        student_name_lb.place(x=5, y=130)

        student_name_ent = tk.Entry(edit_data_page_fm, font=('Bold', 13), highlightcolor='black',
                                    highlightthickness=2, highlightbackground='gray')
        student_name_ent.place(x=5, y=155, width=180, height=30)
        ...
        student_age_ent = tk.Entry(edit_data_page_fm, font=('Bold', 13,), highlightcolor='black',
                                   highlightthickness=2, highlightbackground='gray', )
        student_age_ent.place(x=130, y=200, width=55, height=30)
        ...
        student_contact_lb = tk.Label(edit_data_page_fm, text='Student Contact Number ',
                                      font=('times new roman', 13))
        student_contact_lb.place(x=5, y=245)

        student_contact_ent = tk.Entry(edit_data_page_fm, font=('Bold', 13,), highlightcolor='black',
                                       highlightthickness=2, highlightbackground='gray', )
        student_contact_ent.place(x=5, y=270, width=180, height=30)
        ...
        student_year_lb = tk.Label(edit_data_page_fm, text='Your Year ',
                                   font=('times new roman', 13))
        student_year_lb.place(x=5, y=310)

        select_year_btn = Combobox(edit_data_page_fm, font=('times new roman', 13),
                                   state='readonly', values=year_list)
        select_year_btn.place(x=5, y=335, width=180, height=30)

        select_year_btn.set(get_student_details[0][3])

        student_branch_lb = tk.Label(edit_data_page_fm, text='Your Branch ',
                                     font=('times new roman', 13))
        student_branch_lb.place(x=5, y=375)

        select_branch_btn = Combobox(edit_data_page_fm, font=('times new roman', 13),
                                     state='readonly', values=branch_list)
        select_branch_btn.place(x=5, y=400, width=180, height=30)

        select_branch_btn.set(get_student_details[0][4])

        student_email_lb = tk.Label(edit_data_page_fm, text='Email Address : ',
                                    font=('times new roman', 13))
        student_email_lb.place(x=5, y=440)

        student_email_ent = tk.Entry(edit_data_page_fm, font=('Bold', 13,),
                                     highlightcolor='black', highlightthickness=2,
                                     highlightbackground='gray', )
        student_email_ent.place(x=5, y=465, width=200, height=30)
        student_email_ent.bind('<KeyRelease>',
                               lambda e: remove_highlight_warning(entry=student_email_ent))

        student_email_ent.insert(tk.END, get_student_details[0][6])

        update_data_btn = tk.Button(edit_data_page_fm, text='Update', font=('Bold', 15),
                                    fg='white', bg=bg_color, bd=0,
                                    command=check_inputs)
        update_data_btn.place(x=240, y=510, width=80)
...
def student_login_page():
    # password show and hide in student page
    def show_hide_password():
    ...
    def forward_to_welcome_page():  # '<-' button student page to welcome page
    ...
    back_btn = tk.Button(student_login_page_fm, text='←', font=('Bold', 20),
                         fg='black', bd=0, command=forward_to_welcome_page)
...
def admin_dashboard():
    ...
    announcement_btn.place(x=10, y=171)
    ...
    def home_page():
        ...
        year_list_lb = tk.Label(home_page_fm, text='Number of Students By Branch',
                                font=('Bold', 13), bg=bg_color, fg='white')
        year_list_lb.place(x=20, y=130)

        students_numbers_lb = tk.Label(home_page_fm, text='', font=('Bold', 15),
                                       justify=tk.LEFT)
        students_numbers_lb.place(x=20, y=170)

        for i in year_list:
            for j in branch_list:
                result = fetch_student_data(query=f"SELECT COUNT(*) FROM data WHERE year == '{i}' AND branch == '{j}'")

                students_numbers_lb['text'] += f"{i} Year {j} Branch:         {result[0][0]}\n\n"
    ...
    def find_student_page():
        ...
        def find_student():
            ...
            SELECT id_number, name, year, branch, gender FROM data 
        ...
        def generate_student_card():
            ...
            get_student_details = fetch_student_data(f"""
                SELECT name, age, gender, year, branch, phone_number, email FROM data WHERE id_number == '{selected_id}'
                """)
...
{get_student_details[0][6]}
        ...
        search_filters = ['id', 'name', 'year', 'branch', 'gender']
        ...
        find_by_option_btn.place(x=85, y=53, width=80)
        find_by_option_btn.set('id')
        ...
        record_table['columns'] = ('id', 'name', 'year', 'branch', 'gender')
        ...
        record_table.column('year', width=30, anchor=tk.W)

        record_table.heading('branch', text='Branch', anchor=tk.W)
        record_table.column('branch', width=30, anchor=tk.W)

        record_table.heading('gender', text='Gender', anchor=tk.W)
        record_table.column('gender', width=30, anchor=tk.W)
    ...
    def announcement_page():
        ...
        def add_y_b(year, branch):

            pair = (year, branch)

            if pair in selected_y_bs:
                selected_y_bs.remove(pair)
            else:
                selected_y_bs.append(pair)
        ...
        def collect_emails():
            ...
            for year, branch in selected_y_bs:
                    emails = fetch_student_data(f"SELECT email FROM data WHERE year = '{year}' AND branch = '{branch}' ")
        ...
        def send_announcement(email_addresses):
            box_fm = tk.Frame(root,highlightbackground=bg_color,
                              highlightthickness=3)
            heading_lb = tk.Label(box_fm, text='Sending Emails', font=('Bold', 15),
                                  bg=bg_color, fg='white')
            ...
            for email in email_addresses:
                ...
                sending_lb.config(text=f"Sending To:\n{email}\n\n{sent_count}/{len(email_addresses)}")
            box_fm.destroy()
            message_box(message="Annoucement Sent\nSuccessfully")
        ...
        years_list_lb = tk.Label(announcement_page_fm, text='Select Classes to Announcement',
                                 font=('Bold', 12))
        years_list_lb.place(x=10, y=310)

        y_position = 350
        for year in year_list:
            for branch in branch_list:
                def checkbutton_command(year=year, branch=branch):
                    add_y_b(year, branch)

                year_check_btn = tk.Checkbutton(announcement_page_fm, text=f'year {year} branch {branch}',
                                                command=checkbutton_command)
                year_check_btn.place(x=10, y=y_position)
                y_position += 25

        send_announcement_btn = tk.Button(announcement_page_fm, text='Send Announcement',
                                          font=('Bold', 12), bg=bg_color, fg='white',
                                          command=collect_emails)
        send_announcement_btn.place(x=180, y=525, height=20)
...
def admin_login_page():
    # password show and hide in admin login page
    def show_hide_password():
        if password_ent['show'] == '*':
            password_ent.config(show='')
            show_hide_btn.config(image=unlocked_icon)
    ...
    username_lb = tk.Label(admin_login_page_fm, text='Enter Admin User Name', font=('Bold', 16), fg=bg_color)
    ...
    password_lb = tk.Label(admin_login_page_fm, text='Enter Admin Password', font=('Bold', 16), fg=bg_color)
    password_lb.place(x=80, y=240)
...
def add_account_page():
    ...
    def check_input_validation():
        if student_name_ent.get() == '':
            ...
            message_box(message='Select Student branch')
            ...
            student_email_ent.config(highlightcolor='red',
                                     highlightbackground='red')
            student_email_ent.focus()
            message_box(message='Please Enter a Vaild\nEmail Address')
            ...
            if pic_path.get() != '':
                ...
                read_data = open('graduated.png', 'rb')
                pic_data = read_data.read()
                read_data.close()
                pic_path.set('graduated.png')
            ...
            add_data(id_number=student_id.get(),
                     ...
                     student_year=select_year_btn.get(),
...
"""
    ...
    add_account_page_fm = tk.Frame(root, highlightbackground='black',
                                   highlightthickness=3)

    add_pic_section_fm = tk.Frame(add_account_page_fm, highlightbackground='black',
                                  highlightthickness=2, )
    ...
    student_name_ent = tk.Entry(add_account_page_fm, font=('Bold', 13), highlightcolor='black',
                                highlightthickness=2, highlightbackground='gray')
    student_name_ent.place(x=5, y=160, width=180, height=30)
    student_name_ent.bind('<KeyRelease>',
                          lambda e: remove_highlight_warning(entry=student_name_ent))
    ...
    male_gender_btn = tk.Radiobutton(add_account_page_fm, text='Male',
                                     font=('times new roman', 12), variable=student_gender,
                                     value='male')
    ...
    student_age_ent = tk.Entry(add_account_page_fm, font=('Bold', 13,), highlightcolor='black',
                               highlightthickness=2, highlightbackground='gray', )
    student_age_ent.place(x=150, y=275, width=65, height=30)

    student_age_ent.bind('<KeyRelease>',
                         lambda e: remove_highlight_warning(entry=student_age_ent))

    student_contact_lb = tk.Label(add_account_page_fm, text='Enter Student Contact Number ',
                                  font=('times new roman', 13))
    student_contact_lb.place(x=5, y=330)

    student_contact_ent = tk.Entry(add_account_page_fm, font=('Bold', 13,), highlightcolor='black',
                                   highlightthickness=2, highlightbackground='gray', )
    student_contact_ent.place(x=5, y=360, width=180, height=30)

    student_contact_ent.bind('<KeyRelease>',
                             lambda e: remove_highlight_warning(entry=student_contact_ent))

    student_year_lb = tk.Label(add_account_page_fm, text='Select Your Year ',
                               font=('times new roman', 13))
    student_year_lb.place(x=5, y=400)

    select_year_btn = Combobox(add_account_page_fm, font=('times new roman', 13),
                               state='readonly', values=year_list)
    select_year_btn.place(x=5, y=430, width=180, height=30)

    student_branch_lb = tk.Label(add_account_page_fm, text='Select Your Branch ',
                                 font=('times new roman', 13))
    student_branch_lb.place(x=5, y=465)

    select_branch_btn = Combobox(add_account_page_fm, font=('times new roman', 13),
                                 state='readonly', values=branch_list)
    select_branch_btn.place(x=5, y=495, width=180, height=30)

    student_id_lb = tk.Label(add_account_page_fm, text='Student ID Number :',
                             font=('Times new Roman', 13))
    student_id_lb.place(x=260, y=25)

    student_id = tk.Entry(add_account_page_fm, font=('times new roman', 15), bd=0)
    student_id.place(x=260, y=55, width=120, height=30, )

    student_id.config(state='readonly')

    generate_id_number()

    id_info_lb = tk.Label(add_account_page_fm, text="""Automatically Generated ID Nunber
    ! Remember Using This ID Number
    Student will Login Account.""", justify=tk.LEFT)

    id_info_lb.place(x=260, y=90)

    student_email_lb = tk.Label(add_account_page_fm, text='Enter Email Address : ',
                                font=('times new roman', 13))
    student_email_lb.place(x=260, y=150)

    student_email_ent = tk.Entry(add_account_page_fm, font=('Bold', 13,),
                                 highlightcolor='black', highlightthickness=2,
                                 highlightbackground='gray', )
    student_email_ent.place(x=260, y=180, width=200, height=30)
    student_email_ent.bind('<KeyRelease>',
                           lambda e: remove_highlight_warning(entry=student_email_ent))
    ...
    Studnt will get Future Notification""", justify=tk.LEFT)
    email_info_lb.place(x=260, y=210)

    account_password_lb = tk.Label(add_account_page_fm, text='Create  Account Password', font=('times new roman', 13))
    account_password_lb.place(x=260, y=285)

    account_password_ent = tk.Entry(add_account_page_fm, font=('Bold', 13,),
                                    highlightcolor='black', highlightthickness=2,
                                    highlightbackground='gray', )
    account_password_ent.place(x=260, y=310, width=200, height=30)
    ...
    email_info_lb.place(x=260, y=210)
    account_password_info_lb.place(x=260, y=345)

    home_btn = tk.Button(add_account_page_fm, text="Home", font=('Times new roman', 13),
                         bg='red', fg='white', bd=0, command=forword_to_welcome_page)
    home_btn.place(x=260, y=440)

    Submit_btn = tk.Button(add_account_page_fm, text="Submit", font=('Times new roman', 13),
                           bg='blue', fg='white', bd=0, command=check_input_validation)
    Submit_btn.place(x=370, y=440)
